...
``` bash
# 创建端口
vncserver :31 -geometry 2560x1600 -depth 24     # 创建一个 vnc 服务器端口, 端口号为 31, 分辨率为 2560x1600, 颜色深度为 24 位 (适合高质量图形显示)
...
### 1. PeakView 电感工具导入 (否则无法运行含电感仿真)

参考 XSH 师兄 2026.08.20 给的图片教程：<div class="center"><img src="https://imagebank-0.oss-cn-beijing.aliyuncs.com/VS-PicGo/2026-08-20-18-13-27_如何通过 SSH 远程连接实验室服务器.png"/></div>

我们的步骤如下：
- 在自己的 work 目录下打开终端，输入 `module load peakview`
- 输入 `module av` 查看是否加载成功，能找到 peakview/5.1.6 即为成功 (显示为黑色或者灰色)
- 将服务器上的文件 `/App/env/.cdsinit` 复制到自己的目录下 (随便什么地方，比如我们的是 `/home/dy2025/Desktop/Work/AA_Workspace_tsmcN28/.cdsinit_for_peakview`)，然后将其中的代码复制到工作目录对应的 `.cdsinit` 文件中，代码如下：

``` bash
/* 2026.08.20 
load peakview tool for inductor-related design and simulation
*/
...
;;;;;;;;;alps setup;;;;;;;;;;;;
alps_integration_path =getShellEnvVar("alps_integration_path")
load(strcat(alps_integration_path "/cdsinit_alps"))
;;;;;;;;;;;;;;;;;
;;;;;;;;;Peak setup;;;;;;;;;;;;
loadContext(strcat(getShellEnvVar("PEAKHOME") "/cds/context/pkv.cxt"))
load(strcat(getShellEnvVar("PEAKHOME") "/cds/context/pkview.il"))
;;;;;;;;;;;;;;;;;
```

- 重新打开 virtuoso, CIW 中显示 `PKV: initializing peakdesign done.` 即表示 peakview 加载成功。或者随便打开一个版图，可以在上方菜单栏中找到 `PKView` 一项，即为加载成功。
