...
Cadence Virtuoso 毕竟是一个只能在 Linux 系统上运行的专业软件，大多数主机为 windows 的用户会选择将其安装到合适的虚拟机上 (如 VMware, VirtualBox 等)。此时如何在主机 (windows) 和虚拟机之间传输文件就成了一个问题。虽然可以通过网络传输文件，但这通常比较麻烦，尤其是当虚拟机和主机不在同一局域网时，更别说有的虚拟机完全没有进行网络配置，无法连接网络。
