...
- 多个 `analog block` 会自动拼接到一起，相当于只有一个，因此不需要担心多个 `analog block` 之间的变量作用域问题。
...
### 2.6 macro definition in generate/for loop

下面这样是能正常工作的：

``` verilog
// generate loop:
generate j (1, 7) V(Delay[j]) <+ j/`PHASE_NUM * period; // 


// or for loop:
for (j = 1; j < 7; j = j + 1) begin
    V(Delay[j]) <+ j/`PHASE_NUM * period; // set the delay for each output to be a fraction of the period, to achieve the desired phase shift
end
```
...
/*
`define PHASE_NUM 8
`define PHASE_NUM_minusOne (PHASE_NUM - 1)
*/

// generate loop:
generate j (1, `PHASE_NUM_minusOne) V(Delay[j]) <+ j/`PHASE_NUM * period; // 


// or for loop:
for (j = 1; j < `PHASE_NUM; j = j + 1) begin
    V(Delay[j]) <+ j/`PHASE_NUM * period; // set the delay for each output to be a fraction of the period, to achieve the desired phase shift
end
```

困扰了两三周的问题，2026.02.15 10:44 终于破案了：是因为 `PHASE_NUM` 代表的 8 是 integer-8，`PHASE_NUM_minusOne` 代表的 7 也是 integer-7，所以一做除法 `j/PHASE_NUM` 就变成了 integer-division，结果全是 0，导致所有输出的 delay 都是 0，没有任何 phase shift，仿真结果就不对了。

解决方案便呼之欲出：定义 `PHASE_NUM_integer = 8` 和 `PHASE_NUM_float = 8.0` 两个宏即可，这样在除法时用 `PHASE_NUM_float` 就能得到正确的结果了：
...
详见 [Overview and Verification of CDR Phase and Frequency Detectors](<AnalogIC/Overview and Verification of CDR Phase and Frequency Detectors.md>)。
...
- [SIMETRIX > Verilog-A Manual](https://help.simetrix.co.uk/8.0/simetrix/mergedProjects/verilog_a_reference/topics/veriloga_verilog_areference_verilog_afunctions.htm)